...
def checkout():
    ...
    try:
        ...
        release_db(conn)
    # No PayPal — just pass items and total
...
def checkout_capture():
    ...
    # Use provided mock ID or generate one
    ...
    try:
        ...
        total = sum(
            float(prods[k]["price"]) * v
            for k, v in cart.items()
            if k in prods
        )

        # Store mock_order_id instead of paypal_order_id
        cur.execute(
            "INSERT INTO orders (user_id, total_amount, status, mock_order_id) "
            "VALUES (%s,%s,'paid',%s) RETURNING id",
            (session["user_id"], round(total, 2), mock_order_id)
        )
